...
public class FileUtil {
     ...
     *
     * @param path
     * @param suffxes 是否需要后缀名
     * @return
     */
    public static String getFileName(String path, boolean suffxes) {
        File file = new File(path);
        if (file.exists()) {
            if (suffxes) {
                return file.getName();
     ...
     * @return String
     */
    public static String getFileSuffxe(String path) {
        File file = new File(path);
        if (file.exists()) {
     ...
     */
    public static File findFile(String dirStr, String extensionNoDot) {
        File dirFile = new File(dirStr);
        return findFile(dirFile, extensionNoDot);
    }
    /**
     * 根据后缀名查找文件
     *
     * @param dirFile        目录
     * @param extensionNoDot 后缀名 -不带“.”号，多个后缀名用“;”号隔开
     */
    public static File findFile(File dirFile, String extensionNoDot) {
        File[] files = dirFile.listFiles(new FileNameSelector(extensionNoDot));
        if (files != null && files.length > 0)
            return files[0];

        File[] file2 = dirFile.listFiles(filter-> {
            return filter.isDirectory();
        });
        for (File file : file2) {
            File f = findFile(file, extensionNoDot);
            if (f != null)
                return f;
        }
        return null;
